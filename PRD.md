...
**AUDIVUE** is a real-time AI Vision Assistant that processes a camera feed (smartphone or webcam) using local computer vision to perceive the environment on behalf of visually impaired users and translates visual data into immediate spatial detection alerts and telemetry.
...
1. **Pipeline 1: Obstacle & Object Awareness**
   ...
   - Generates real-time spatial alert telemetry (e.g., *"Chair ahead, slightly left, close"*).
...
2. **Pipeline 2: Currency Detection & Counting**
   ...
   - Detects multiple notes simultaneously in a single frame, extracts individual values, and automatically calculates the total aggregate sum (e.g., *"500, 200, and 100 rupees detected — total 800 rupees"*).
